#!/usr/bin/env python3
"""Outro 1.0.9 verification suite — run 3 times before delivery.
...
check("A4 versionName 1.0.9", "versionName='1.0.9'" in badging)
check("A5 versionCode 10", "versionCode='10'" in badging)
...
# --- 1.0.8 rename checks ---
_app_name = read(os.path.expanduser("~/workspace/jazzy/app/src/main/res/values/strings.xml"))
check("B22 app_name is Outro in strings.xml",
      '<string name="app_name">Outro</string>' in _app_name)
check("B23 application label renders as Outro in badging",
      "application-label:'Outro'" in badging)
_src_all = ""
for _root, _, _files in os.walk(SRC):
    for _fn in _files:
        if _fn.endswith((".kt", ".java")):
            _src_all += read(os.path.join(_root, _fn))
import re as _re
_visible_opus = [l for l in _src_all.splitlines()
                 if re.search(r'"[^"]*\bOpus\b', l)]
check("B24 no user-visible \"Opus\" string literals remain in Kotlin sources",
      not _visible_opus, f"{len(_visible_opus)} hits")
check("B25 CLIENT_NAME is Outro",
      'CLIENT_NAME = "Outro"' in _src("network/SubsonicClient.kt"))
check("B26 SettingsScreen about line says Outro 1.0.9",
      '"Outro 1.0.9' in sett)
check("B27 party guest page branded Outro Party Queue",
      "Outro Party Queue" in party_server)
check("B28 internal package com.opus.music unchanged (upgrade-safe)",
      "com.opus.music" in main and "com.opus.music" in _manifest_txt)

# --- 1.0.9 light theme checks ---
_theme = read(f"{SRC}/ui/theme/Theme.kt")
_mode_ctl = read(f"{SRC}/ui/theme/ThemeMode.kt")

check("D1 lightColorScheme defined in Theme.kt",
      "private val OpusLightColors = lightColorScheme(" in _theme)
check("D2 light theme is clean white (Paper background)",
      "background = Paper" in _theme and "val Paper = Color(0xFFFFFFFF)" in _theme)
check("D3 dark theme unchanged (Midnight background, Brass primary)",
      "background = Midnight" in _theme and "primary = Brass" in _theme)
check("D4 OpusTheme takes darkTheme parameter",
      "fun OpusTheme(darkTheme: Boolean" in _theme
      and "if (darkTheme) OpusDarkColors else OpusLightColors" in _theme)
check("D5 ThemeController holds dark/light mode",
      "object ThemeController" in _mode_ctl
      and 'const val DARK = "dark"' in _mode_ctl
      and 'const val LIGHT = "light"' in _mode_ctl
      and "MutableStateFlow" in _mode_ctl)
check("D6 theme pref key app_theme (dark default, light option)",
      'KEY_THEME = "app_theme"' in repo
      and "fun getThemeMode" in repo and "fun setThemeMode" in repo
      and '"opus_prefs"' in repo)
check("D7 MainActivity seeds theme from prefs and passes darkTheme",
      "ThemeController.init(Graph.settings.getThemeMode())" in main
      and "OpusTheme(darkTheme = themeMode != ThemeController.LIGHT)" in main)
check("D8 Settings Display section has Dark/Light picker dialog",
      '"Theme")' in sett and "setThemeMode" in sett and "ThemeController.set" in sett
      and '"Light"' in sett and '"Dark"' in sett)
_non_theme_src = ""
for _root, _, _files in os.walk(SRC):
    if "ui/theme" in _root.replace(os.sep, "/"):
        continue
    for _fn in _files:
        if _fn.endswith((".kt", ".java")):
            _non_theme_src += read(os.path.join(_root, _fn))
_visible_brass = [l for l in _non_theme_src.splitlines()
                  if "Brass" in l and "import" not in l]
check("D9 no hardcoded Brass color usages left outside theme (theme-aware primary)",
      not _visible_brass, f"{len(_visible_brass)} hits")
check("D10 light primary has contrast on white (BrassDeep)",
      "primary = BrassDeep" in _theme and "val BrassDeep = Color(0xFF9A6B1E)" in _theme)
